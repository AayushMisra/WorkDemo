...
#define DEBUG flag_posit_debug

/*positRounding() - RNE Rounding for Posit 32 and Posit 64
 * Parameters required, r: Pointer to REAL_VALUE_TYPE structure
 * 		       es: No. of exponent bits in Posit
 * 		        m: Consecutive bits of Regime
 * 	       positImage: Formed posit encoding to find LSB
 * 	       	    width: 32 for Posit32, 64 for Posit64
 * Output, RNE: 1 to round, 0 to ignore
 * Written By Aayush Misra <aayushmisra@cdac.in>*/
bool positRounding(const REAL_VALUE_TYPE *r, unsigned int es, unsigned int m, unsigned long pImage, unsigned int width){
	if(DEBUG != 0)
		printf("\n---In Posit Rounding---");
	bool lsb, guard, S=0;
	unsigned long capt, sticky;

	unsigned long sig = r->sig[SIGSZ-1] << 1;
	unsigned long e = (unsigned long)(REAL_EXP(r)-1) << (64 - es); 	
	...
	if(DEBUG != 0)
		printf("guard: %d\n", guard);
      	...
      	for (int i = 0; i < 2; ++i)
		sticky |= r->sig[i];
	if((capt<<1) != 0 || sticky != 0)	
		S = 1;
	if(DEBUG != 0)
	printf("S: %d\n", S);
	
	bool RNE = (guard & (lsb | S));
...
real_from_string (REAL_VALUE_TYPE *r, const char *str)
{
  ...
  if (str[0] == '0' && (str[1] == 'x' || str[1] == 'X'))
	...
	goto is_a_zero;
      ...
      inexact = mpfr_strtofr (m, str, NULL, 10, MPFR_RNDN);//Rounding Mode is Round to nearest, ties to even
	...
	{
	  real_from_mpfr (r, m, NULL_TREE, MPFR_RNDN);//Rounding Mode is Round to nearest, ties to even
...
round_for_format (const struct real_format *fmt, REAL_VALUE_TYPE *r)
{
  ...
  if (r->decimal)
	...
	/*Posit Change: Passing higher significand bits value to 
  	support Posit, can be done from format structure*/
  p2 = 61;//fmt->p;
  emin2m1 = fmt->emin - 1;
  emax2 = fmt->emax;
...
/*encode_ieee_single() - Replacing IEEE754 single precision encoding with Posit 
 * Posit Encoding consisits of four parts: Sign, Regime, Exponent and Significand Calculation
 * Written By Aayush Misra <aayushmisra@cdac.in>*/
static void
...
{
	if(DEBUG != 0){
		printf("--------IEEE Single Format---------\n");
		printf("\nES32 flag value: %d\n", flag_posit_es32);
	}

	unsigned long image, sig, e, sign=r->sign;
        unsigned int m, es=flag_posit_es32;
	bool RNE=0;

	
	//---m: No. of consecutive bits in regime value----
	//Checking for positive and negative exponent, and also internally making the overflow and underflow correction
	int k=0;
	int rexp = REAL_EXP(r) - 1;
	k = rexp>>es;
	if (rexp >= 0){
		if(k > 30)
			m = 31;
	...
	//---Significand Bits Extraction from real_value---
	//The MSB includes the hidden bit in the compiler real number representation 	
	sig = r->sig[SIGSZ-1] << 1;
	if((m+2+es) < 32)
 		sig = sig >> (32+m+2+es);
	else
        ...
        switch(r->cl){
			...
			image = 0x80000000;
                        break;
                case rvc_normal:
			//---Significand positioning in image---
			image = sig;
			if(DEBUG != 0)
				printf("Image after sig: %lx\n", image);	

 			//---Exponent positioning in image---
			e = (unsigned int)(REAL_EXP(r)-1) << (32-es);
			if((m+2) < 32)
 				e >>= (m+2);
			else
				e = 0;
			image |= e;
			if(DEBUG != 0)
				printf("Image after exp: %lx\n", image);
 
			//---Regime positioning in image---
			//Checking for positive or negative exponent for consecutive 1's or 0's in regime
			if((REAL_EXP(r)-1) < 0){
				image |= (1 << (30-m));
			}else{
				int shift = (int)0x80000000 >> m;
				shift &= 0x7fffffff;
				image |= shift;
			}
			if(DEBUG != 0)
				printf("Image after regime: %lx\n", image);
			
			//---Rounding Posit---
			if(k>-31 && k<30)
				RNE = positRounding(r, es, m, image, 32);
			if(RNE)
				image += 1;
			
			//---Handling Negative Numbers---
			//If sign is negative we need to perform 2's complement
			if(sign){
				image = ~image;
				image += 1;
			}	
...
//----For Testing Posit 32-----------------------	
	if(DEBUG != 0){
		static int flag=1;
		FILE *fp;
		if(flag == 1){
			fp = fopen("/home/misra/posit_dataT2-32-2.txt","w");
			flag=0;
		}else{
			fp = fopen("/home/misra/posit_dataT2-32-2.txt","a");
		}
		fprintf(fp,"%08lx\n", image);
		fclose(fp);
	}
...
/* encode_ieee_double() - Replacing Poist Encoding instead of IEEE754 double precision 
 * Posit Encoding consisits of four parts: Sign, Regime, Exponent and Significand
 * Written By Aayush Misra <aayushmisra@cdac.in>*/
static void
...
{
	if (DEBUG!=0){
		printf("-----In IEEE Double Format---------\n");
		printf("\nES64 flag value: %d\n", flag_posit_es64);
	}
	
	bool RNE = 0;
	unsigned long image, image_lo, image_hi, sig, e;
	unsigned int m, es = flag_posit_es64;
...
//---m: No. of consecutive bits in regime value----
	...
	k = rexp>>es;
	if(rexp >= 0){
		if(k > 62)//Overflow Correction
			m = 63;
		else
			m = k+1; 
	}else{
		if(k < -62)//Underflow Correction
			m = 62;
		else
			m = -k;
	}
...
switch (r->cl){
	case rvc_zero:
    	...
    	case rvc_inf:
		if(r->sign)
			image = 0x8000000000000001;
		else
			image = 0x7fffffffffffffff;
      		break;

    	case rvc_nan:
		image = 0x8000000000000000;
      		break;
		...
		//---Exponent positioning in image---
		e = (unsigned long)(REAL_EXP(r)-1) << (64-es);
		if((m+2) < 64)
 			e >>= (m+2);
		else
			e = 0;
		...
		//---Regime positioning in image---
 		// For negative exponent setting 1 at appropriate location
 		// For positive exponent setting consecutive 1's 	
		if((REAL_EXP(r)-1) < 0)
			image |= ((unsigned long)1 << (62-m));
		else{
			long int shift = (long int)0x8000000000000000 >> m;
			shift = shift & 0x7fffffffffffffff;
			image |= shift;
		}	
		if (DEBUG!=0)
			printf("Image after reg: %lx\n", image);	
		
		//---Posit Rounding---
		//We check the boundaries of k, if within we forward for rounding
		if(k>-63 && k<62)
			RNE = positRounding(r, es, m, image, 64);
		if(RNE)
			image += 1;
...
//---File Generation----------------------
	if (DEBUG!=0){
		...
		FILE *fp;
		if(flag == 1){
			fp = fopen("/home/misra/posit_dataT2-64-3.txt","w");
			flag=0;
		}else{
			fp = fopen("/home/misra/posit_dataT2-64-3.txt","a");
		}
		fprintf(fp,"%016lx\n", image);
		fclose(fp);
	}

//---Dividing Final Image------------------  
	image_hi = (image >> 32);
	image_lo = (image & 0xffffffff);
